...
class ADBClient:
    ...
    def start_intent(self, uri: str) -> bool|None:
        command = 'am start -a android.intent.action.VIEW -d {}'.format(uri)
        Logger.info(f'Starting intent: [bold green]{uri}[/bold green] ..')
        result = self.execute_shell_command(command)
        if 'Error' in result:
            Logger.error(f'Starting intent [bold blue]{uri}[/bold blue] failed')
            return False
        else:
            Logger.success(f'App: [bold blue]{uri}[/bold blue] started successfully')
            return True
